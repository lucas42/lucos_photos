...
@router.put("/people/{person_id}/profile-picture", status_code=status.HTTP_200_OK)
async def set_profile_picture(
    ...
    """Pin a specific photo as a person's profile picture (lucas42/lucos_photos#473).

    Validates the chosen photo actually contains a face belonging to this person —
    otherwise an unrelated photo could be pinned deliberately. The crop itself is
    generated by the worker (generate_manual_profile_picture): cropping/re-encoding an
    image is exactly the "image compression" work this API process must not do per
    this repo's API/worker boundary, so this endpoint only writes the DB override and
    enqueues the job.
    """
    try:
        person_uuid = uuid.UUID(person_id)
    ...
    photo_id_str = body.get("photoId")
    if not photo_id_str:
        raise HTTPException(status_code=422, detail="photoId is required")
    try:
        photo_uuid = uuid.UUID(str(photo_id_str))
    except ValueError:
        raise HTTPException(status_code=422, detail="photoId must be a valid UUID")

    face_exists = db.query(Face).filter(Face.person_id == person_uuid, Face.photo_id == photo_uuid).first()
    if not face_exists:
        raise HTTPException(status_code=422, detail="Chosen photo does not contain a face belonging to this person")

    person.profile_photo_id = photo_uuid
    person.profile_auto_generated = False
    db.commit()
    db.refresh(person)

    _enqueue_generate_manual_profile_picture(str(person_uuid), str(photo_uuid))

    await emit_loganne_event("profilePictureSet", f"Profile picture manually set for {person.display_name or str(person_uuid)} in lucos_photos")

    return person_to_dict(person)


@router.delete("/people/{person_id}/profile-picture", status_code=status.HTTP_200_OK)
async def revert_profile_picture(
    person_id: str,
    _: Annotated[None, Depends(verify_session_or_key)],
    db: Session = Depends(get_db),
):
    """Revert a manually-pinned profile picture back to automatic selection."""
    try:
        person_uuid = uuid.UUID(person_id)
    except ValueError:
        raise HTTPException(status_code=404, detail="Person not found")
    ...
    person.profile_auto_generated = None
    db.commit()
    db.refresh(person)

    _enqueue_profile_picture(str(person_uuid))

    await emit_loganne_event("profilePictureReverted", f"Profile picture reverted to automatic for {person.display_name or str(person_uuid)} in lucos_photos")
...
def _enqueue_generate_manual_profile_picture(person_id: str, photo_id: str) -> None:
    """Enqueue generate_manual_profile_picture for a manually-pinned photo. Non-fatal
    on Redis errors."""
    try:
        from app.redis_client import get_redis
        from rq import Queue
        from rq.job import Retry
        from lucos_photos_common.jobs import generate_manual_profile_picture
        redis_conn = get_redis()
        queue = Queue("photos", connection=redis_conn)
        queue.enqueue(generate_manual_profile_picture, person_id, photo_id, retry=Retry(max=3, interval=[10, 30, 60]))
    except Exception as exc:
        print(f"Warning: failed to enqueue generate_manual_profile_picture for {person_id}: {exc}", flush=True)
