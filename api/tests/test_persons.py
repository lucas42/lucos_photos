...
class TestSetProfilePicture:
    """Tests for PUT/DELETE /people/{id}/profile-picture (lucas42/lucos_photos#473 —
    the "Set as profile picture" / "Revert to automatic" actions)."""

    def _make_face(self, db, photo, person):
        face = Face(
            photo_id=photo.id,
            person_id=person.id,
            person_confirmed=True,
    ...
    def test_pins_profile_picture(self, authenticated_client, db_session):
        person = make_person(db_session, "Alice")
        photo = make_photo(db_session, "f1" * 32)
        self._make_face(db_session, photo, person)
        db_session.commit()

        with patch("app.routers.people.emit_loganne_event", new_callable=AsyncMock) as mock_emit, \
             patch("app.routers.people._enqueue_generate_manual_profile_picture") as mock_enqueue:
            response = authenticated_client.put(
                f"/people/{person.id}/profile-picture",
                json={"photoId": str(photo.id)},
            )

        assert response.status_code == 200
        data = response.json()
        assert data["profileAutoGenerated"] is False

        db_session.refresh(person)
        assert person.profile_photo_id == photo.id
        assert person.profile_auto_generated is False

        mock_enqueue.assert_called_once_with(str(person.id), str(photo.id))
        mock_emit.assert_called_once()
        assert mock_emit.call_args[0][0] == "profilePictureSet"

    def test_rejects_photo_with_no_face_for_person(self, authenticated_client, db_session):
        """The strawberry case from #473 — a photo with no face for this person must
        be rejected, not silently pinned."""
        person = make_person(db_session, "Alice")
        photo = make_photo(db_session, "f2" * 32)
        db_session.commit()
        ...
        db_session.refresh(person)
        assert person.profile_photo_id is None

    def test_rejects_face_belonging_to_a_different_person(self, authenticated_client, db_session):
        person = make_person(db_session, "Alice")
        other_person = make_person(db_session, "Bob")
        photo = make_photo(db_session, "f3" * 32)
        self._make_face(db_session, photo, other_person)  # face is Bob's, not Alice's
        db_session.commit()
    ...
    def test_person_not_found(self, authenticated_client, db_session):
        photo = make_photo(db_session, "f4" * 32)
        db_session.commit()
        response = authenticated_client.put(
            f"/people/{uuid.uuid4()}/profile-picture",
            json={"photoId": str(photo.id)},
        )
        assert response.status_code == 404

    def test_photo_id_required(self, authenticated_client, db_session):
        person = make_person(db_session, "Alice")
        db_session.commit()
        response = authenticated_client.put(f"/people/{person.id}/profile-picture", json={})
        assert response.status_code == 422

    def test_invalid_photo_id(self, authenticated_client, db_session):
        person = make_person(db_session, "Alice")
        db_session.commit()
        response = authenticated_client.put(
            f"/people/{person.id}/profile-picture",
            json={"photoId": "not-a-uuid"},
        )
        assert response.status_code == 422

    def test_requires_authentication(self, client, db_session):
        person = make_person(db_session, "Alice")
        photo = make_photo(db_session, "f5" * 32)
        db_session.commit()
        response = client.put(
            f"/people/{person.id}/profile-picture",
            json={"photoId": str(photo.id)},
    ...
    def test_reverts_profile_picture(self, authenticated_client, db_session):
        photo = make_photo(db_session, "f6" * 32)
        ...
        with patch("app.routers.people.emit_loganne_event", new_callable=AsyncMock) as mock_emit, \
             patch("app.routers.people._enqueue_profile_picture") as mock_enqueue:
            response = authenticated_client.delete(f"/people/{person.id}/profile-picture")

        assert response.status_code == 200
        data = response.json()
        assert data["profileAutoGenerated"] is None

        db_session.refresh(person)
        assert person.profile_auto_generated is None

        mock_enqueue.assert_called_once_with(str(person.id))
        mock_emit.assert_called_once()
        assert mock_emit.call_args[0][0] == "profilePictureReverted"

    def test_revert_person_not_found(self, authenticated_client):
        response = authenticated_client.delete(f"/people/{uuid.uuid4()}/profile-picture")
        assert response.status_code == 404

    def test_revert_requires_authentication(self, client, db_session):
        person = make_person(db_session, "Alice")
        db_session.commit()
        response = client.delete(
            f"/people/{person.id}/profile-picture",
    ...
    def test_serializer_includes_profile_auto_generated(self, authenticated_client, db_session):
        make_person(db_session, "Alice")
        db_session.commit()
        ...
        assert response.status_code == 200
        person_data = response.json()["people"][0]
        assert "profileAutoGenerated" in person_data
        assert person_data["profileAutoGenerated"] is None
