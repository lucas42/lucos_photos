...
class TestListPhotos:
    ...
    def test_returns_empty_list_when_no_photos(self, client):
        response = client.get("/photos", headers=AUTH_HEADER)
        assert response.status_code == 200
        data = response.json()
        assert data["photos"] == []
        assert data["total"] == 0

    def test_returns_list_of_photos(self, client, db_session):
    ...
    def test_photo_has_expected_fields(self, client, db_session):
    ...
    def test_pagination_limit(self, client, db_session):
        ...
        response = client.get("/photos?limit=3", headers=AUTH_HEADER)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_pagination_offset(self, client, db_session):
        ...
        response = client.get("/photos?limit=3&offset=3", headers=AUTH_HEADER)
        assert response.status_code == 200
        data = response.json()
        assert len(data["photos"]) == 2
        assert data["offset"] == 3

    def test_default_order_is_uploaded_at_descending(self, client, db_session):
...
class TestGetPhoto:
    ...
    def test_returns_404_for_unknown_id(self, client):
        response = client.get(f"/photos/{uuid.uuid4()}", headers=AUTH_HEADER)
        assert response.status_code == 404

    def test_returns_404_for_invalid_uuid(self, client):
        response = client.get("/photos/not-a-uuid", headers=AUTH_HEADER)
        assert response.status_code == 404

    def test_returns_photo_metadata(self, client, db_session):
        photo = make_photo(db_session, "gettest")
        db_session.commit()

        response = client.get(f"/photos/{photo.id}", headers=AUTH_HEADER)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_includes_processing_status(self, client, db_session):
    ...
    def test_processing_status_is_none_when_not_set(self, client, db_session):
    ...
    def test_includes_faces(self, client, db_session):
    ...
    def test_face_includes_person_id_when_tagged(self, client, db_session):
    ...
    def test_includes_persons_list(self, client, db_session):
    ...
    def test_persons_empty_when_no_tags(self, client, db_session):
...
class TestGetPhotoFile:
    ...
    def test_returns_404_for_unknown_photo(self, client):
        response = client.get(f"/photos/{uuid.uuid4()}/file", headers=AUTH_HEADER)
        assert response.status_code == 404

    def test_returns_404_for_invalid_uuid(self, client):
        response = client.get("/photos/not-a-uuid/file", headers=AUTH_HEADER)
        assert response.status_code == 404

    def test_returns_404_when_file_not_on_disk(self, client, db_session, monkeypatch, tmp_path):
        ...
        response = client.get(f"/photos/{photo.id}/file", headers=AUTH_HEADER)
        assert response.status_code == 404

    def test_serves_derivative_by_default(self, client, db_session, monkeypatch, tmp_path):
        import app.main as main_module
        monkeypatch.setattr(main_module, "PHOTOS_DIR", tmp_path)
    ...
    def test_falls_back_to_original_when_no_derivative(self, client, db_session, monkeypatch, tmp_path):
        import app.main as main_module
        monkeypatch.setattr(main_module, "PHOTOS_DIR", tmp_path)
    ...
    def test_serves_original_when_requested(self, client, db_session, monkeypatch, tmp_path):
        import app.main as main_module
        monkeypatch.setattr(main_module, "PHOTOS_DIR", tmp_path)
        ...
        response = client.get(f"/photos/{photo.id}/file?original=true", headers=AUTH_HEADER)
        assert response.status_code == 200
        assert response.content == b"original_content"

    def test_sets_correct_content_type_for_jpg(self, client, db_session, monkeypatch, tmp_path):
        import app.main as main_module
        monkeypatch.setattr(main_module, "PHOTOS_DIR", tmp_path)
    ...
    def test_sets_cache_control_header(self, client, db_session, monkeypatch, tmp_path):
        import app.main as main_module
        monkeypatch.setattr(main_module, "PHOTOS_DIR", tmp_path)
