...
class TestRefreshContactDisplayName:
    ...
    def test_constructs_url_from_db_contact_id(self, db_session, monkeypatch):
        """HTTP request URL is built from LUCOS_CONTACTS_URL + person.contact_id, not from user input."""
        monkeypatch.setenv("LUCOS_CONTACTS_URL", "https://contacts.example.com")
        monkeypatch.setenv("KEY_LUCOS_CONTACTS", "testkey")
    ...
    def test_skips_when_env_vars_unset(self, db_session, monkeypatch):
        """Returns early if LUCOS_CONTACTS_URL or KEY_LUCOS_CONTACTS are missing."""
